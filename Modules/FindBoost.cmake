...
IF(APPLE)
    ...
    IF(BOOST_VERSION_REQUIRED)
       # Check if Homebrew Boost library is installed in /usr/local/opt/boost@${BOOST_VERSION_REQUIRED}/
	   MESSAGE("Checking for Boost (Homebrew) in /usr/local/opt/boost@${BOOST_VERSION_REQUIRED}/")
       IF(EXISTS /usr/local/opt/boost@${BOOST_VERSION_REQUIRED}/)	
       	   SET(Boost_LIBRARY_DIR /usr/local/opt/boost@${BOOST_VERSION_REQUIRED}/lib/)
   		   SET(Boost_INCLUDE_DIR /usr/local/opt/boost@${BOOST_VERSION_REQUIRED}/include/)
   		   SET(Boost_LIBRARY_DIRS /usr/local/opt/boost@${BOOST_VERSION_REQUIRED}/lib/)
  		   SET(Boost_INCLUDE_DIRS /usr/local/opt/boost@${BOOST_VERSION_REQUIRED}/include/)
  		   INCLUDE_DIRECTORIES(${Boost_INCLUDE_DIRS})
           LINK_DIRECTORIES(${Boost_LIBRARY_DIRS})
  	       SET(BOOST_FOUND 1)
 	   ELSE()
   		   MESSAGE(FATAL_ERROR "Homebrew version of Boost in /usr/local/opt/boost@${BOOST_VERSION_REQUIRED}/ not found!")
   		   SET(BOOST_FOUND 0)
	   ENDIF()    
    ELSE()
